...
import { kundenStore, useEntityList } from "@/lib/entityStore";
import { fmtDate } from "@/lib/format";
import { STATUS_KUNDEN, statusColor } from "@/lib/status";
...
export default function KundenPage() {
  ...
  function updateEntity(id: string, patch: Partial<Entity>) {
    const list = kundenStore.getSnapshot();
    const idx = list.findIndex((c) => c.id === id);
    if (idx < 0) return;
    const next = [...list];
    next[idx] = { ...next[idx], ...patch };
    kundenStore.set(next);
  }

  function deleteEntity(id: string, firma: string) {
    if (!confirm(`${firma} wirklich löschen?`)) return;
    kundenStore.set(kundenStore.getSnapshot().filter((c) => c.id !== id));
    setExpandedId(null);
  }
  ...
  function handleCreate(e: React.FormEvent<HTMLFormElement>) {
    ...
    const entry: Entity = {
      ...
      kanton: String(fd.get("kanton") || ""),
      branche: String(fd.get("branche") || ""),
      quelle: String(fd.get("quelle") || ""),
      status: String(fd.get("status") || "Neu"),
      ...
      history: [],
      nextAction: null,
      statusHistory: [{ status: String(fd.get("status") || "Neu"), datum: now }],
    };
    kundenStore.set([...kundenStore.getSnapshot(), entry]);
  ...
  return (
    <div className="flex min-h-screen flex-col">
        ...
        <div className="mb-5 flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-0.5 text-sm text-mute">
              <span>{filtered.length}</span> von <span>{all.length}</span> Kontakten
            </p>
          </div>
          ...
          <table className="w-full min-w-[880px] border-collapse text-sm">
            <thead className="border-b border-line bg-paper-2 text-xs uppercase tracking-wide text-mute">
              <tr>
                ...
                <th className="px-4 py-2.5 text-left font-medium">Ort</th>
      ...
      <Modal open={modalOpen} title="Neuer Kunde" onClose={() => setModalOpen(false)}>
  ...
  return (
    <>
      <tr onClick={onToggle} className="cursor-pointer transition-colors hover:bg-paper-2">
        ...
        <td className="px-4 py-3 text-mute">{c.ort || "–"}</td>
        <td className="px-4 py-3 text-mute">{c.branche || "–"}</td>
        <td className="px-4 py-3">
          <StatusBadge status={c.status} color={statusColor(STATUS_KUNDEN, c.status)} />
        </td>
        <td className="px-4 py-3">
